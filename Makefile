...
LOCAL_AUTH_COMPOSE := $(COMPOSE) -f $(PROJECT_DIR)/docker-compose.yml -f $(PROJECT_DIR)/docker-compose.local-auth.yml --project-directory $(PROJECT_DIR)

.PHONY: help \
	fix lint test e2e e2e-up e2e-down e2e-local-auth check check-frontend check-all qa \
...
help:
	...
	@printf "  %-18s %s\n" "e2e-local-auth" "Run Playwright smoke test against local auth mode"
...
e2e-local-auth:
	set -e; \
	$(LOCAL_AUTH_COMPOSE) up --build -d; \
	trap '$(LOCAL_AUTH_COMPOSE) down -v' EXIT; \
	cd $(FRONTEND_DIR) && PLAYWRIGHT_AUTH_MODE=local PLAYWRIGHT_BASE_URL="$${PLAYWRIGHT_BASE_URL:-http://localhost:$${LOCAL_AUTH_FRONTEND_PORT:-5175}}" npm run e2e -- e2e/local-auth.spec.ts
